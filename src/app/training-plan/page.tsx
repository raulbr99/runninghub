...
export default function TrainingPlanPage() {
  ...
  const editPlan = async () => {
    ...
    try {
      const response = await fetch('/api/training-plan/edit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          plan,
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Error al editar el plan');
      }

      const updatedPlan = await response.json();
      setPlan(updatedPlan);
      setChatHistory(prev => [...prev, { role: 'assistant', content: 'Plan actualizado correctamente.' }]);
    } catch (err) {
        ...
        <div className="space-y-6">
          ...
          <div className="bg-zinc-900/50 rounded-xl border border-zinc-800/50 overflow-hidden">
            <div className="p-4 border-b border-zinc-800/50">
              <h3 className="text-sm font-light text-zinc-100 uppercase tracking-wider flex items-center gap-2">
                <svg className="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              ...
              <p className="text-xs text-zinc-500 mt-1">
                Pide cambios al plan: "quita la sesion de fuerza del martes", "reduce el volumen de la semana 3"...
              </p>
            </div>

            {/* Chat history */}
            {chatHistory.length > 0 && (
              <div className="p-4 space-y-3 max-h-48 overflow-y-auto border-b border-zinc-800/50">
            ...
            <div className="p-4">
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && !isEditing && editPlan()}
                  placeholder="Escribe tu cambio..."
                  disabled={isEditing}
                  className="flex-1 p-3 border border-zinc-700/50 rounded-lg bg-zinc-800/50 text-zinc-100 placeholder-zinc-600 focus:ring-1 focus:ring-emerald-500/50 disabled:opacity-50"
                />
                <button
                  onClick={editPlan}
                  disabled={isEditing || !chatMessage.trim()}
                  className="px-4 py-3 bg-emerald-600 hover:bg-emerald-500 disabled:bg-zinc-700 text-white rounded-lg transition-colors"
                >
                  {isEditing ? (
